...
//get all companies
_publics.getAllCompanies = (search) => {
    return new Promise((resolve, reject) => {
        ...
        if (search !== undefined && search !== "" && search !== null) {
            ...
            //var condition = "( (CONCAT(LOWER(name), ' ', LOWER(code), ' ', LOWER(email)) like " + "'%" + input + "%'" + ") )"
            var condition = "(( name is not null and (LOWER(name) like " + "'%" + input + "%'" + ") ) " +
                " or ( email is not null and (LOWER(email) like " + "'%" + input + "%'" + ") ) " +
                " or ( code is not null and (LOWER(code) like " + "'%" + input + "%'" + ") )) ";
...
_publics.updateCompany = (req, company) => {
    ...
    return new Promise((resolve, reject) => {
        var msg = "";
        var sql = "update company set name=?,country=?,zip_code=?,city=?,adress=?,country_code=?,phone=?,logo=?,category=?,email=? where id=?";
        pool.getConnection(function(err, connection) {
            if (err) {
                reject(err);
            }
            connection.query(sql, [name, country, zip_code, city, adress, country_code, phone, logo, category, email, id], function(err, result) {
                connection.release();
                if (err) {
                    msg = "failure";
                    reject(err);
