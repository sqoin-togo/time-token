...
var input = "";
//get all users 
_publics.getAllUsers = (search) => {

    return new Promise((resolve, reject) => {
        var sql = "select m.* FROM user m where role is null ";
        if (search !== undefined && search !== "" && search !== null) {
            input = search.toLowerCase();
            var condition = " and (( firstname is not null and (LOWER(firstname) like " + "'%" + input + "%'" + ") ) " +
                " or ( email is not null and (LOWER(email) like " + "'%" + input + "%'" + ") ) " +
                " or ( lastname is not null and (LOWER(lastname) like " + "'%" + input + "%'" + ") )) ";
            sql = sql + condition;
        }
...
_publics.updateUserByEmail = (req, user) => {
    ...
    return new Promise((resolve, reject) => {
        var msg = "";
        var sql = "UPDATE user SET firstname=?,lastname=?,birth_date=?,city=? WHERE email=?";
        pool.getConnection(function(err, connection) {
            if (err) {
                reject(err);
            }
            connection.query(sql, [firstname, lastname, birth_date, city, email], function(err, result) {
                connection.release();
                if (err) {
                    msg = "failure";
                    reject(err);
...
_publics.updateUserPhotoByEmail = (req, user) => {
    var user = JSON.parse(user);
    var photo = user.photo;
    var email = req.query.email;
    return new Promise((resolve, reject) => {
        var msg = "";
        var sql = "UPDATE user SET photo=? WHERE email = ?";
        pool.getConnection(function(err, connection) {
            if (err) {
                reject(err);
            }
            connection.query(sql, [photo, email], function(err, result) {
                connection.release();
                if (err) {
                    msg = "failure";
                    reject(err);
...
//get userId by email from oauth database
_publics.getUserIdByEmail = (req) => {
    var emailUser = req.query.email;
    return new Promise((resolve, reject) => {
        var sql = "select id from users where email=$1";
        dbpool.connect(function(err, connection, release) {
            if (err) {
                reject(err);
            }
            connection.query(sql, [emailUser], function(err, result) {
