...
_publics.getActivitiesDetails = (search) => {
    return new Promise((resolve, reject) => {
        var input = "";
        ...
        if (search !== undefined && search !== "" && search !== null) {
            ...
            var condition = "(( u.firstname is not null and (LOWER(u.firstname) like " + "'%" + input + "%'" + ") ) " +
                " or ( a.name is not null and (LOWER(a.name) like " + "'%" + input + "%'" + ") ) " +
                " or ( u.lastname is not null and (LOWER(u.lastname) like " + "'%" + input + "%'" + ") )) ";
    ...
    return new Promise((resolve, reject) => {
        ...
        pool.getConnection(function(err, connection) {
            ...
            connection.query(sql, [id], function(err, result) {
                connection.release();
                if (err) reject(err);
                if (result.length !== 0) { return resolve(JSON.stringify(result[0])); } else return resolve(null);
            });
        });
    });
};
