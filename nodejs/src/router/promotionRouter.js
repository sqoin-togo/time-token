...
const { search } = require('./userRouter');
...
router.get('/getAllPromotions', urlencodedParser, (req, res, next) =>
    promotionController.getAllPromotions(req.query.search)
        .then(promotions => {
            res.send(promotions)
        })
        .catch(next));
...
router.get('/getallImagePromotions', urlencodedParser, (req, res, next) =>
    promotionController.getallImagesPromotions()
        .then(promotions => {
            res.send(promotions)
        })
        .catch(next));
...
router.get('/getPromotionsDetails', urlencodedParser, (req, res, next) =>
    promotionController.getPromotionsDetails(req.query.search)
        .then(promotions => {
            res.send(promotions)
        })
        .catch(next));
